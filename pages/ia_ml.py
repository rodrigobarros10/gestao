...
setup_page(layout="wide", page_title="IA & ML", page_icon="🤖")

apply_ultra_compact_css()

require_access(page_keys=["ia_ml", "ia_avancada"])
...
if st.session_state.get('show_expanded_ia'):
    with st.container(border=True):
        ...
        with col_t:
            st.markdown(
                f"<h3 style='color:white; margin-top:5px;'>🔍 {st.session_state.get('expanded_ia_title', 'Visão Ampliada')}</h3>",
...
try:
    df_meses = run_query(engine, "SELECT DISTINCT TO_CHAR(data, 'YYYY-MM') as mes FROM public.vw_resumo_bilhetagem ORDER BY 1 DESC")
    lista_meses = df_meses['mes'].dropna().tolist() if not df_meses.empty else [datetime.today().strftime('%Y-%m')]
except Exception:
    lista_meses = [datetime.today().strftime('%Y-%m')]

# UI do Filtro
col_filtro, _ = st.columns([2, 8])
with col_filtro:
    mes_selecionado = st.selectbox("📅 Mês de Referência:", lista_meses, key="filtro_mes_ia")
...
# ==============================================================================
# FILEIRA 1: PROJEÇÕES (1 ANO) - HISTÓRICO ATÉ O MÊS SELECIONADO
# ==============================================================================
df_pax = run_query(engine, f"SELECT TO_CHAR(data, 'YYYY-MM-01')::date as mes, SUM(total_passageiros) as qtd FROM public.vw_resumo_bilhetagem WHERE TO_CHAR(data, 'YYYY-MM') <= '{mes_selecionado}' GROUP BY 1 ORDER BY 1")
df_viagens = run_query(engine, f"SELECT TO_CHAR(data, 'YYYY-MM-01')::date as mes, SUM(total_viagens) as qtd FROM public.vw_resumo_viagens WHERE TO_CHAR(data, 'YYYY-MM') <= '{mes_selecionado}' GROUP BY 1 ORDER BY 1")
...
with c_pax:
    with st.container(border=True):
        ...
        if h_dates_p:
            ...
            render_chart_footer(pd.DataFrame({'Data': f_dates_p, 'Projecao': f_y_p}), "proj_pax", fig_p, "Projeção de Passageiros", "exp_pax", state_prefix="expanded_ia")
...
with c_trips:
    with st.container(border=True):
        ...
        if h_dates_v:
            ...
            render_chart_footer(pd.DataFrame({'Data': f_dates_v, 'Projecao': f_y_v}), "proj_viag", fig_v, "Projeção de Viagens", "exp_viag", state_prefix="expanded_ia")

# ==============================================================================
# FILEIRA 2: FALHAS, ATRASOS E ANOMALIAS - FILTRADO EXATAMENTE NO MÊS
# ==============================================================================
c_fal, c_atr, c_ano = st.columns(3)

with c_fal:
    with st.container(border=True):
        st.markdown("<div class='pbi-title'>🛠️ Previsão de Falha TUEs no Mês</div>", unsafe_allow_html=True)
        df_falhas = run_query(engine, f"SELECT id_tue::text as \"Trem\", SUM(total_manutencoes) as qtd FROM public.vw_resumo_manutencao WHERE tipo_manutencao='CORRETIVA' AND TO_CHAR(data, 'YYYY-MM') = '{mes_selecionado}' GROUP BY 1 ORDER BY 2 DESC")
        if not df_falhas.empty:
            ...
            render_chart_footer(df_falhas, "risco_falhas", fig_f, "Previsão de Falha Crítica na Frota", "exp_fal", state_prefix="expanded_ia")

with c_atr:
    with st.container(border=True):
        st.markdown("<div class='pbi-title'>⏱️ Mapa de Calor: Headway (Origem vs Hora)</div>", unsafe_allow_html=True)
        df_hw = run_query(engine, f"SELECT hora, origem, AVG(headway) as hw FROM public.vw_headway_diario_hora WHERE TO_CHAR(data, 'YYYY-MM') = '{mes_selecionado}' GROUP BY 1, 2")
        if not df_hw.empty:
            ...
            render_chart_footer(df_pivot.reset_index(), "mapa_headway", fig_hm, "Mapa Real de Headways e Atrasos", "exp_atr", state_prefix="expanded_ia")

with c_ano:
    with st.container(border=True):
        st.markdown("<div class='pbi-title'>🚨 Detecção de Anomalias Reais (Z-Score)</div>", unsafe_allow_html=True)
        df_ano = run_query(engine, f"SELECT hora, SUM(total_passageiros) as fluxo FROM public.vw_resumo_bilhetagem WHERE TO_CHAR(data, 'YYYY-MM') = '{mes_selecionado}' GROUP BY 1 ORDER BY 1")
        if not df_ano.empty:
            ...
            render_chart_footer(df_ano, "anomalias_reais", fig_an, "Sistema de Detecção de Anomalias (Z-Score)", "exp_ano", state_prefix="expanded_ia")

# ==============================================================================
# FILEIRA 3: ENERGIA, FRAUDES E RADAR 
# ==============================================================================
...
with c_ene:
    with st.container(border=True):
        ...
        # Mantém até o mês para criar a linha temporal de até 6 meses
        df_km = run_query(engine, f"SELECT TO_CHAR(mes_ref, 'MM/YYYY') as mes, SUM(km_total) as km FROM public.vw_resumo_frota WHERE TO_CHAR(mes_ref, 'YYYY-MM') <= '{mes_selecionado}' GROUP BY 1 ORDER BY MIN(mes_ref) DESC LIMIT 6")
        if not df_km.empty:
            ...
            render_chart_footer(pd.DataFrame({'Mes': df_km['mes'], 'Real': consumo_real, 'Otimizado': consumo_otimizado}), "energia_ml", fig_en, "Otimização Energética Baseada em KM", "exp_ene", state_prefix="expanded_ia")

with c_fra:
    with st.container(border=True):
        st.markdown("<div class='pbi-title'>🕵️ Clusterização: Gratuidades Suspeitas no Mês</div>", unsafe_allow_html=True)
        df_grat = run_query(engine, f"SELECT id_estacao, SUM(total_passageiros) as qtd FROM public.vw_resumo_bilhetagem WHERE grupo_bilhetagem ILIKE '%Gratuidade%' AND TO_CHAR(data, 'YYYY-MM') = '{mes_selecionado}' GROUP BY 1")
        if not df_grat.empty:
            ...
            render_chart_footer(df_grat, "fraudes_gratuidades", fig_cl, "Prevenção de Evasão em Gratuidades", "exp_fra", state_prefix="expanded_ia")

with c_rad:
    with st.container(border=True):
        st.markdown("<div class='pbi-title'>⚠️ Mapeamento Preditivo de Risco no Mês</div>", unsafe_allow_html=True)
        df_rad = run_query(engine, f"SELECT subtipo as Categoria, SUM(total_ocorrencias) as Risco FROM public.vw_resumo_ocorrencias WHERE TO_CHAR(data, 'YYYY-MM') = '{mes_selecionado}' GROUP BY 1 ORDER BY 2 DESC LIMIT 6")
        if not df_rad.empty:
            ...
            render_chart_footer(df_rad, "radar_risco", fig_rad, "Radar Analítico de Ocorrências e Segurança", "exp_rad", state_prefix="expanded_ia")
...
with c_scipy:
    with st.container(border=True):
        ...
        df_stat = run_query(engine, f"SELECT SUM(total_viagens) as viagens, AVG(tempo_medio_minutos) as tempo FROM public.vw_resumo_viagens WHERE tempo_medio_minutos > 0 AND TO_CHAR(data, 'YYYY-MM') = '{mes_selecionado}' GROUP BY data")
...
with c_sk:
    with st.container(border=True):
        ...
        # Mantém o histórico para o modelo ser capaz de treinar
        df_rf = run_query(engine, f"SELECT km_total as km, horas_indisp as hr FROM public.vw_resumo_frota WHERE km_total > 0 AND TO_CHAR(mes_ref, 'YYYY-MM') <= '{mes_selecionado}'")
