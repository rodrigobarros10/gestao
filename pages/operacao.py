...
with col_c3:
    with st.container(border=True):
        ...
        if not df_inc.empty:
            ...
            fig_inc.update_layout(height=350, margin=dict(t=10, b=0, l=0, r=0), xaxis_title="Tipo de Ocorrência", yaxis_title="Qtd", xaxis=dict(color="#FFFFFF"), yaxis=dict(color="#FFFFFF"), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
...
with st.container(border=True):
    st.markdown("<div class='pbi-title'>🕒 Headway Médio (min e seg)</div>", unsafe_allow_html=True)
    q_hw = f"""
    SELECT 
        hora, 
    ...
    if not df_hw.empty:
        ...
        fig_hw = px.line(df_hw, x='hora', y='headway', markers=True, text='headway_texto')
        fig_hw.update_traces(textposition="top center", textfont=dict(color="#FFFFFF"))
        fig_hw.update_layout(height=300, margin=dict(l=0, r=0, t=10, b=0), xaxis_title="Hora do Dia", yaxis_title="Tempo de Intervalo", xaxis=dict(color="#FFFFFF", tickmode='linear'), yaxis=dict(color="#FFFFFF"), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
...
with st.container(border=True):
    ...
    if not df_turno.empty:
            ...
            fig_turno.update_layout(margin=dict(t=10, b=0, l=0, r=0), legend=dict(font=dict(color="#FFFFFF")), xaxis=dict(color="#FFFFFF"), yaxis=dict(color="#FFFFFF"), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
